...
float Simulator::simulate()
{
	...
	//perform selection and generation of new population (empty spots left for elitism)
	for (int i = NO_OF_ELITES; i < pop_size_; i++) {
	...
	std::vector<float> fitness_copy(fitness_);
	for (int i = 0; i < NO_OF_ELITES; i++) {
		int index = std::min_element(fitness_copy.begin(), fitness_copy.end()) - fitness_copy.begin();
		elite[i] = index;

		//make sure the element is not found during the next iteration of the loop
		fitness_copy[index] = FLT_MAX;
	}

	//by the principle of elitism, the best solutions gets to stay in the gene pool
	for (int i = 0; i < NO_OF_ELITES; i++) {
		new_population_[i] = new Phenotype(*population_[elite[i]]);
	}
...
std::vector<std::pair<std::string, float>> Simulator::getNBest(int n) const
{
	...
	for (int i = 0; i < n; i++)
	{
		returnValue[i] = std::make_pair(solutions[i].first->toString(), solutions[i].second);
	}

	return returnValue;
}

std::string Simulator::getCurrentBest() const
{
...
float Simulator::fitnessStDev() const
{
	float avg = averageFitness();
	float rms = 0;
	for (int i = 0; i < pop_size_; i++) {
		rms += pow(fitness_[i] - avg, 2);
	}
	rms = avg / pop_size_;
	rms = sqrt(rms);
...
std::pair<int, int> Simulator::selection(Selection strategy)
{
	...
	//higher precision than usual is necessary
	std::vector<double> normalized_fitness;
	double total_fitness, threshold1, threshold2, s, min_fitness, max_fitness, width;
	...
	case Selection::ROULETTE:
		...
		for (double f : normalized_fitness) {
			total_fitness += f;
		}

		threshold1 = rand->UniformDouble(0, total_fitness);
		threshold2 = rand->UniformDouble(0, total_fitness);
